...
datum/Map_Regenerator/proc/GenerateMapSave()
	...
	for(var/Y = 1, Y <= world.maxy, Y++)
		for(var/X = 1, X <= world.maxx, X++)
			...
			if(!active_turf)
				continue
			for(var/obj/structure/window/W in active_turf)
				if(istype(W, /obj/structure/window/reinforced))
					output["/obj/structure/window/reinforced/[X]/[Y]/[W.dir]"] << 1
				else
					output["/obj/structure/window/[X]/[Y]/[W.dir]"] << 1
			if(!last_turf_type)
	...
	for(var/iy = miny, iy<= maxy, iy++)
		for(var/ix = minx, ix <= maxx, ix++)
			...
			if(T.name == "UPDATEPOINT")
				...
				T = new active_type(T)
			for(var/atom/A in view(8, T))
				if(A.luminosity)
					var/Lum = (A.luminosity-get_dist(A,T))
					if(Lum < 0) continue
					T.update_lumcount(Lum)
...
//					world << "Direction: [d]"
					...
					W.dir = text2num(d)
			save.cd = "/obj/structure/window/[ix]/[iy]"
//			world << "CD: [save.cd]"
//			world << "Len: [length(save.dir)]"
			if(length(save.dir != 0))
				for(var/d in save.dir)
