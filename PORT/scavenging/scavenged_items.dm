...
/obj/scavenging/item
	engine
		...
		setup_item()
			...
			add_fixedreward(/obj/item/stack/cable_coil)
			add_fixedreward(pick(typesof(/obj/item/stack/sheet/mineral)))
	...
	oldcryopod
		...
		setup_item()
			...
			add_randomreward(/obj/item/stack/medical/bruise_pack)
			add_randomreward(/obj/item/stack/medical/ointment)
	...
	oldnucleargen
		...
		setup_item()
			...
			add_fixedreward(/obj/item/constructionkit/component/control_board)
			add_randomreward(/obj/item/weapon/stock_parts/cell)
	...
	thermalgenerator
		...
		setup_item()
			...
			add_fixedreward(pick(typesof(/obj/item/stack/sheet/mineral)))
			add_fixedreward(/obj/item/stack/cable_coil)
			add_fixedreward(/obj/item/weapon/stock_parts/cell)
			add_fixedreward(/obj/item/weapon/stock_parts/cell)
			...
			add_randomreward(/obj/item/weapon/stock_parts/cell/high)
			add_randomreward(/obj/item/weapon/stock_parts/cell/super)
	...
	infiniteimprobabilitydrive
		...
		setup_item()
			...
			add_fixedreward(/obj/item/constructionkit/component/diode)
			add_fixedreward(/obj/item/constructionkit/component/mini_fusion_reactor)
			add_randomreward(/obj/item/weapon/reagent_containers/food/snacks/grown/banana)
			add_randomreward(/obj/item/weapon/stock_parts/cell/super)
			add_randomreward(/obj/item/weapon/reagent_containers/food/drinks/milk)
			add_randomreward(/obj/item/weapon/reagent_containers/food/drinks/bottle/vodka)
			add_randomreward(/obj/item/weapon/reagent_containers/food/drinks/beer)
			add_randomreward(/obj/item/weapon/reagent_containers/food/drinks/drinkingglass/cola)
			add_randomreward(/obj/item/weapon/reagent_containers/food/drinks/coffee)
			add_randomreward(/obj/item/clothing/head/cakehat)
			add_randomreward(/obj/item/clothing/head/hardhat/orange)
		...
		handle_event()
			...
			if(random <= 60)
				//event()
				//BASICALLY SPAWN SHITTON OF DESTRUCTION.
				visible_message("A wave of improbability is released!")
			else
				visible_message("The drive misfires!")
				world << "<font colour='blue'>A wave of improbability washes over the station!</font>"
				var/locations = list()
...
//			add_fixed_reward( //REWARD NEEDED
	...
	cargopod
		...
		setup_item()
			...
			add_sequence_item(/obj/item/weapon/stock_parts/cell,"You insert a new cell.")
		...
		handle_state_change(obj/item/tool)
			...
			if(state == 11) //Handle reward distribution here, so we can specify probabilities.
				...
				if(probability < 10)
					...
					new /obj/item/weapon/gun/energy/xray(src.loc)
				else if(probability < 40)
					new /obj/item/weapon/storage/box/emps(src.loc)
					for(var/i = 1;i <= 5; i++)
						new /obj/item/weapon/grenade/chem_grenade/incendiary(src.loc)
					...
					for(var/i=1;i <= rand(1,3);i++)
						new /obj/item/clothing/mask/facehugger(src.loc)
